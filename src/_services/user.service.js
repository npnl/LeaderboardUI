  ...
  return fetch(`${serverConstants.BASE_URL}/login`, requestOptions)
    ...
    .then(response_data => {
      ...
      if (response_data && response_data.session && response_data.session.access_token) {
        ...
        localStorage.setItem('access_token_leaderboard', response_data.session.access_token);
...
function logout() {
  ...
  localStorage.removeItem('access_token_leaderboard');
...
function makeSubmission(submission) {
  ...
  return fetch(`${serverConstants.BASE_URL}/submissions`, requestOptions).then(handleResponse);
}
